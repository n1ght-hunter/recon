...
impl OutputHardwareDevice {
    ...
    pub unsafe fn set_master_volume_scaler(self, volume: f32) -> Result<(), String> {
        self.audio_endpoint_volume
            .SetMasterVolumeLevelScalar(0.1, ptr::null())
            .map_err(|e| format!("{:?}", e))
    }
    /// set volume to decibels
    ///
    /// use get db range to see range of dbs
    pub unsafe fn set_master_volume_db(self, volume: f32) -> Result<(), String> {
        self.audio_endpoint_volume
            .SetMasterVolumeLevel(0.1, ptr::null())
            .map_err(|e| format!("{:?}", e))
    }
}
...
unsafe fn initialize_audio_endpoint(device: &IMMDevice) -> IAudioEndpointVolume {
    ...
    device
        .Activate::<IAudioEndpointVolume>(CLSCTX_ALL, prop)
...
unsafe fn get_sessions(
    ...
    Ok((0..sessions)
        ...
        .collect::<Vec<IAudioSessionControl2>>())
}
...
unsafe fn get_mixer_sessions(
    ...
    for session in get_sessions(session_manager).unwrap().into_iter() {
