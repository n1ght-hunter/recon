...
pub trait WindowsCaptureHandler: Sized {
        ...
        Self: std::marker::Send + 'static,
    {
        ...
        unsafe {
            while GetMessageW(&mut message, None, 0, 0).into()  && !stop.load(std::sync::atomic::Ordering::Relaxed) {
        ...
        unsafe {
            while GetMessageW(&mut msg, None, 0, 0).into() && !stop.load(std::sync::atomic::Ordering::Relaxed) {
